...
export const ScanInstance: React.FC = () => {
  ...
  // ============================================
  // PASO 1: Cargar datos de Appwrite
  // ============================================
  useEffect(() => {
    const fetchInstanceData = async () => {
      if (!instanceName) {
        setError('Nombre de instancia no válido');
        return;
      }
      ...
      try {
        ...
        const response = await databases.listDocuments(
          DATABASE_ID,
          COLLECTION_ID,
          [Query.equal('instance_name', instanceName)]
        );
        ...
        const doc = response.documents[0] as unknown as InstanceData;
        ...
        // Iniciar obtención de QR (NO recrear instancia)
        startQrRefresh(instanceName);
        
      } catch (err: any) {
        console.error('❌ Error al cargar datos:', err);
        setError(err.message || 'Error al cargar datos de la instancia');
        notify.error({
          message: 'Error',
  ...
  // ============================================
  // PASO 2: Manejar conexión exitosa
  // ============================================
  useEffect(() => {
  ...
  const updateInstanceStatus = async (name: string) => {
    try {
      const response = await databases.listDocuments(
        DATABASE_ID,
        COLLECTION_ID,
        [Query.equal('instance_name', name)]
      );
      ...
      if (response.documents.length > 0) {
        ...
        console.log('✅ Estado actualizado a connected en Appwrite');
      }
    } catch (err) {
  ...
  // ============================================
  // SOLO obtener QR (NO recrear instancia)
  // ============================================
  const fetchQr = async (name: string) => {
    try {
      ...
      console.log('📥 Respuesta QR:', { hasCode: !!data?.code, hasBase64: !!data?.base64 });
  ...
  const checkStatus = async (name: string) => {
    try {
      ...
      if (state && ["open", "connected", "authenticated"].includes(state)) {
        stopQrRefresh();
        setIsConnected(true);
      }
    } catch (err) {
